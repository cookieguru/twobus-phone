...
if(!isset($directions->Direction[$_REQUEST['Digits']-1])) {
	...
	echo '<Redirect>' . SITE_URL . "6/direction-handler.php?agency={$_REQUEST['agency']}&amp;route={$_REQUEST['route']}</Redirect>";
	exit('</Response>');
}

$max_length = strlen(count($directions->Direction[$_REQUEST['Digits']-1]) + 10);
echo '<Gather action="' . SITE_URL . "6/direction-index-handler.php?agency={$_REQUEST['agency']}&amp;route={$_REQUEST['route']}&amp;direction={$_REQUEST['Digits']}\" numDigits=\"$max_length\">";
$i = 10;
